...
class PostOrderIter(AbstractIter[_T]):
    """
    Iterate over tree applying post-order strategy starting at `node`.

    >>> from tree import Node, RenderTree, AsciiStyle, PostOrderIter
    >>> f = Node("f")
    >>> b = Node("b", parent=f)
    >>> a = Node("a", parent=b)
    >>> d = Node("d", parent=b)
    ...
    def _iter(self, children, filter_, stop, maxlevel) -> t.Generator[_T, None, None]:
    ...
    def __next(children, level, filter_, stop, maxlevel):
        if not AbstractIter._abort_at_level(level, maxlevel):
            for child in children:
                ...
                for grandchild in PostOrderIter.__next(
